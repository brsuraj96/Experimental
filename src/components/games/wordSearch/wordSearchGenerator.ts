...
// Direction mappings
const directions: Array<WordSearchWord["direction"]> = [
  "horizontal",
  "vertical",
  "diagonal-right",
  "diagonal-left",
 ...
 */
export const generateWordSearchLevel = (
  ...
  // Create empty board
  let board: WordSearchCell[][] = Array(gridSize)
    .fill(0)
    .map((_, rowIndex) =>
      Array(gridSize)
        .fill(0)
...
const placeWordOnBoard = (
  ...
  for (const direction of shuffledDirections) {
    ...
    for (let attempts = 0; attempts < 10; attempts++) {
      const { row, col, canPlace } = canPlaceWordAt(
        board,
        word,
        direction,
        gridSize
